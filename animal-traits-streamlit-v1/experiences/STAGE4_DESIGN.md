...
| Using and trusting a model | 7 | Test the model: cat | Test models on a domestic cat. Compact Class / Order / Family / Genus context supports judging model relevance; it is contextual information, not a taxonomy lesson. The mammal model is worked first as the stable anchor: cat body mass is an input, its prediction is model-derived, and the cat brain-mass comparison is separate external evidence hidden until after that prediction. Interpolation or extrapolation is calculated relative to each model's fitted evidence range. Only then do the two comparison models reappear; they inherit Screen 6 choices but remain editable. For each current comparison model, learners commit to whether its cat prediction will be closer to, farther from, or about the same distance from the independent cat evidence as the mammal model, with a brief reason, before its numerical prediction can be revealed. Absolute prediction error reports performance on this cat only; it does not establish universal model superiority. Changing one model invalidates only that model's judgement, reason and reveal. Current stable choices carry forward to Screen 8. There is no long written response, regression diagnostics or R² lesson. |
| Using and trusting a model | 8 | Test the model: elephant | Test the same models on the repository-backed African savanna elephant (*Loxodonta africana*) as a harder case. Compact taxonomy context supports judging relevance. The mammal model is worked first, then its separate elephant brain-mass evidence remains hidden until after prediction. Elephant body mass is an input; interpolation or extrapolation is calculated against each fitted model's own evidence range. Learners identify extrapolation as a reason for extra caution, not automatic invalidation. The two editable comparison models use the same closer / farther / about-the-same-distance prediction and brief-reason gate as Screen 7; absolute error reports performance on this elephant only, never universal superiority. A short cat-interpolation versus elephant-extrapolation contrast bridges to Screen 9. |
...
| Using and trusting a model | 10 | Predict when the answer is unknown | Open by naming the absence of an answer key, then start with a personal-interest animal discovery route and commit to one AnimalTraits species with a prepared species-level body-mass value but no brain-mass value to reveal. **Decision:** before selection, discovery uses compact clickable choices showing common name and, where layout permits, scientific name; detailed taxonomy is revealed only after selection, where it supports reasoning about relevant evidence groups. Learners can return to the chooser and select another animal. Before choosing, learners compare compact summaries of every biologically applicable available model (all animals; Mammal, Bird or Reptile class; then eligible matching Order, Family and Genus evidence): evidence identity, usable paired-species count, and whether the chosen animal sits inside or beyond that model’s body-mass evidence range. Only the selected model receives a fitted evidence visual. Learners choose evidence without seeing a numerical prediction, judge High / Some / Low confidence without scoring, and give at least one judgement-oriented reason; the dynamic inside/beyond range reason never presents contradictory choices, and an `Another reason` alone needs a short explanation. They then make the model-based prediction. There is no correct-answer reveal: the ending makes explicit that the outcome is a prediction, its evidence, and reasons for how much confidence to place in it. |
